...
private:
    ...
    void InitializeCustomComponent()
    {
        ...
        comboBoxCommands->Items->Add("03 - Read Holding Registers");
        comboBoxCommands->Items->Add("04 - Read Input Registers");
        comboBoxCommands->Items->Add("10 - Write Multiple Registers");
...
#pragma region Windows Form Designer generated code
    ...
    void InitializeComponent(void)
    {
        ...
        this->groupBoxCommands->Size = System::Drawing::Size(585, 276);
...
#pragma endregion
    ...
    void SendModbusCommand()
    {
        ...
        uint16_t quantity = (uint16_t)numericQuantity->Value;
        ...
        switch (commandIndex)
        {
        case 0: // 03 - Read Holding Registers
        ...
        case 1: // 04 - Read Input Registers
        ...
        case 2: // 10 - Write Multiple Registers
        {
            ...
            request.push_back((quantity >> 8) & 0xFF);
            request.push_back(quantity & 0xFF);
            request.push_back(quantity * 2); // Количество байт данных

            // Парсим данные из текстового поля
            if (!ParseWriteData(request, quantity))
            {
        ...
        // Вычисляем CRC16 с использованием двух таблиц
        uint16_t crc = CalculateCRC16(request.data(), request.size());

        // В Modbus CRC отправляется младшим байтом первым
        ...
        LogMessage(L"Отправлено байт: " + bytesWritten);

        // Читаем ответ
        uint8_t response[256];
        DWORD bytesRead;
        if (ReadFile(hSerial, response, sizeof(response), &bytesRead, NULL) && bytesRead > 0)
        {
            std::cout << "Получено байт:" << bytesRead << "\nДанные: ";
            for (DWORD i = 0; i < bytesRead; i++) printf("%02X ", response[i]);
        }
        else
        {
    ...
    void SendCommandWithTimeout(const std::vector<uint8_t>& request, uint8_t* response, DWORD* responseSize, DWORD timeoutMs)
    {
        *responseSize = 0;
        ...
        {
            if (ReadFile(hSerial, response + totalBytesRead, 256 - totalBytesRead, &bytesRead, NULL))
            {
                if (bytesRead > 0)
                {
                    ...
                    // Минимальный ответ: адрес(1) + функция(1) + данные(1) + CRC(2) = 5 байт
                    if (totalBytesRead >= 5)
                    {
                        // Проверяем CRC чтобы убедиться что ответ полный
    ...
    bool ParseWriteData(std::vector<uint8_t>& request, uint16_t quantity)
    {
        String^ dataStr = textBoxWriteData->Text->Trim();
        if (dataStr == L"" || dataStr == L"Например: 1234 5678 (hex)")
        {
            // Если данные не введены, используем значения по умолчанию
            for (int i = 0; i < quantity; i++)
            {
                request.push_back(0x12); // Старший байт
                request.push_back(0x34); // Младший байт
            }
            return true;
        }

        // Разбиваем строку по пробелам
        array<String^>^ hexValues = dataStr->Split(' ');
        int expectedBytes = quantity * 2;

        if (hexValues->Length < expectedBytes)
        {
            LogMessage(L"Ожидается " + expectedBytes + L" байт данных, получено " + hexValues->Length);
            return false;
        }
        ...
        for (int i = 0; i < expectedBytes; i++)
        {
            try
            {
                ...
                int value = Convert::ToInt32(hexValues[i], 16);
                request.push_back(static_cast<uint8_t>(value & 0xFF));
            }
            catch (Exception^)
            {
                LogMessage(L"Ошибка парсинга значения: " + hexValues[i]);
                return false;
            }
        }
    ...
    void DisplayRequest(const std::vector<uint8_t>& request)
    {
        ...
        ss << L"Запрос (" << request.size() << L" байт): ";

        for (size_t i = 0; i < request.size(); i++)
        {
            ss << std::hex << std::setw(2) << std::setfill(L'0') << (int)request[i] << L" ";
            // Каждые 8 байт добавляем перенос для удобства чтения
            if ((i + 1) % 8 == 0 && i != request.size() - 1)
                ss << L"\r\n                ";
        }

        // Добавляем информацию о CRC
        if (request.size() >= 2)
        {
            ss << L"\r\nCRC: "
    ...
    void ScanForDevices()
    {
        ...
        // Отключаем кнопки во время сканирования
        ...
        // Устанавливаем специальные таймауты для сканирования
        COMMTIMEOUTS originalTimeouts;
        GetCommTimeouts(hSerial, &originalTimeouts);

        COMMTIMEOUTS scanTimeouts;
        scanTimeouts.ReadIntervalTimeout = 50;          // Максимальное время между байтами
        scanTimeouts.ReadTotalTimeoutConstant = 100;     // Постоянный таймаут
        scanTimeouts.ReadTotalTimeoutMultiplier = 0;     // Не умножаем на количество байт
        scanTimeouts.WriteTotalTimeoutConstant = 100;    // Таймаут записи
        scanTimeouts.WriteTotalTimeoutMultiplier = 0;

        SetCommTimeouts(hSerial, &scanTimeouts);

        // Создаем запрос для чтения 1 регистра
        std::vector<uint8_t> request;
        request.resize(8); // Фиксированный размер для запроса
        ...
        for (int slaveID = MIN_SLAVE_ID; slaveID <= MAX_SLAVE_ID; slaveID++)
        {
            ...
            request[2] = 0x00;  // Старший байт адреса
            request[3] = 0x00;  // Младший байт адреса
            request[4] = 0x00;  // Старший байт количества
            request[5] = 0x01;  // Младший байт количества (1 регистр)
            ...
            if (!WriteFile(hSerial, request.data(), request.size(), &bytesWritten, NULL))
            {
                LogMessage(L"Ошибка отправки запроса для ID " + slaveID);
                continue;
            }
            ...
            // Читаем ответ с таймаутом
            while (GetTickCount64() - startTime < 200) // 200 мс таймаут
            {
                    ...
                    sizeof(response) - totalBytesRead, &bytesRead, NULL))
                {
                    if (bytesRead > 0)
                    {
                        ...
                        // Проверяем минимальный размер ответа (минимум 5 байт: адрес, функция, данные, CRC16)
                        if (totalBytesRead >= 5)
                        {
                            // Проверяем, что это ответ от данного slave ID
            ...
            // Обновляем прогресс в статусе (каждые 10 ID)
            if (slaveID % 10 == 0)
            {
                labelStatus->Text = L"Сканирование... " + (slaveID * 100 / MAX_SLAVE_ID) + L"%";
                Application::DoEvents(); // Позволяем интерфейсу обновиться
            }
        ...
        // Восстанавливаем оригинальные таймауты
